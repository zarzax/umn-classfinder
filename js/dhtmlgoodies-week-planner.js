...
/* User variables */
var headerDateFormat = '';	// Format of day, month in header, i.e. at the right of days
...
function initScheduler(xml)
{
	clearAppointments();
	initWeekScheduler(xml);
}
...
function parseItems(xmlText)
{
	...
	var itemsToBeCreated = new Array();
	var items = xmlText.split(/<item>/g);
	...
	for(var no=1;no<items.length;no++){
		...
		if(itemsToBeCreated[no]['id'])
		{
			...
			var tempLeftPos = (el_x - appointmentsOffsetLeft);
			if(!isNaN(tempLeftPos))
			{

			currentAppointmentDiv = createNewAppointmentDiv(tempLeftPos,topPos,(newAppointmentWidth-(appointmentMarginSize*2)),itemsToBeCreated[no]['description'],elHeight);		
			currentAppointmentDiv.id = itemsToBeCreated[no]['id'];
			currentZIndex = currentZIndex + 1;
			currentAppointmentDiv.style.zIndex = currentZIndex;
			currentTimeDiv = getCurrentTimeDiv(currentAppointmentDiv);
...
function getItemsFromServer(xmlText)
{
	var xmlTemp = "<?xml version=\"1.0\" ?>\n<item>\n	<id>1</id>\n	<description>ACCT 3001 LEC</description>\n	<eventStartDate>Mon, 13 Feb 2006 12:45 CST</eventStartDate>\n	<eventEndDate>Mon, 13 Feb 2006 15:00 CST</eventEndDate>\n	<bgColorCode>#FDAE15</bgColorCode>\n</item>\n<item>\n	<id>2</id>\n	<description>ACCT 3001 LEC</description>\n<eventStartDate>Wed, 15 Feb 2006 12:45 CST</eventStartDate>\n	<eventEndDate>Wed, 15 Feb 2006 15:00 CST</eventEndDate>\n	<bgColorCode>#FDAE15</bgColorCode>\n</item>";
	parseItems(xmlText);
}
...
function createNewAppointmentDiv(leftPos,topPos,width,contentHTML,height)
{
	...
	// header.onmousedown = initMoveAppointment; // uncomment to allow movable appointments
	// header.style.cursor = 'move'; // uncomment to allow movable appointments
...
function getLeftPos(inputObj)
{
	...
	returnValue = returnValue - 307; // Width of the left-margin (5px) + Course List (300px) + Course List Border (2px)
	// alert(returnValue);
  	return returnValue;
}
...
function initWeekScheduler(xmlText)
{
	...
	for(var no=0;no<subDivs.length;no++){
		if(subDivs[no].className=='weekScheduler_appointmentHour'){
			// subDivs[no].onmousedown = initNewAppointment; // uncomment to allow time blocks
